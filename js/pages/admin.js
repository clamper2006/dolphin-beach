/* ================================================================
   pages/admin.js — Administration Page Controller
   ================================================================ */
...
const AdminPage = (() => {
      ...
      <tr>
        ...
        <td>
          <div class="d-flex gap-2">
            ...
            ${u.id !== session?.id ? `
              <button class="btn btn-sm" style="background:var(--danger-light);color:var(--danger);border-color:var(--danger)"
                      onclick="AdminPage.eliminarUsuario(${u.id})" title="Eliminar">
  ...
  function eliminarUsuario(id) {
    app.confirm('¿Eliminar este usuario?', 'Confirmar eliminación', () => {
  ...
  function _guardarUsuario() {
    ...
    if (id) {
      ...
      if (res.success) { Toast.success('Usuario actualizado'); app.closeModal('modal-usuario'); _renderUsuarios(); _editingUserId = null; }
      else Toast.error(res.error);
    } else {
      // Create
      if (!password) { Toast.error('La contraseña es obligatoria'); return; }
      const res = Auth.createUser({ nombre, username, password, rol });
      if (res.success) { Toast.success('Usuario creado'); app.closeModal('modal-usuario'); _renderUsuarios(); }
      else Toast.error(res.error);
    }
  }
  ...
  function _loadSettings() {
    ...
    setVal('cfg-precio-roja',    s.precioRoja     || 15);
    setVal('cfg-precio-premium', s.precioPremium  || 40);
    setVal('cfg-precio-silla',   s.precioSilla    || 5);
  ...
  function _guardarConfig() {
    ...
    s.precioRoja    = getNum('cfg-precio-roja',     15);
    s.precioPremium = getNum('cfg-precio-premium',  40);
    s.precioSilla   = getNum('cfg-precio-silla',    5);
    s.tasaCambio    = getNum('cfg-tasa-cambio',     36.5);
    Storage.saveSettings(s);
    Toast.success('Configuración guardada. Recarga para aplicar cambios de precio.');
  }
  ...
  function _exportar() {
    ...
    a.href      = url;
    a.download  = `dolphin-beach-backup-${new Date().toISOString().split('T')[0]}.json`;
    a.click();
  ...
  function _limpiarSistema() {
    app.confirm(
      'Esto reiniciará TODAS las churuatas a estado disponible. Los registros de entrada no se borrarán.',
      '⚠️ Confirmar limpieza',
      () => {
        // Reset churuatas — _inicializar now clears arrays before repopulating
        if (app.sistema) {
          app.sistema._inicializar();
          Storage.saveChuruatas(app.sistema.exportar());
        }
        if (app.sistemaIsla) {
          app.sistemaIsla._inicializar();
          Storage.saveIsla(app.sistemaIsla.exportar());
        }
